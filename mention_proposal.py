...
class MentionProposalModel(object):
    ...
    def get_mention_proposal_and_loss(
        ...
        start_scores = tf.cast(tf.reshape(tf.sigmoid(tf.math.log(start_scores)), [-1]),tf.float32)
        end_scores = tf.cast(tf.reshape(tf.sigmoid(tf.math.log(end_scores)), [-1]),tf.float32)
