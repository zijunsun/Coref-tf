...
class MentionProposalModel(object):
    ...
    def get_mention_proposal_and_loss(
        ...
        start_scores = tf.math.log(tf.cast(tf.reshape(tf.sigmoid(start_scores), [-1]),tf.float32))
        end_scores = tf.math.log(tf.cast(tf.reshape(tf.sigmoid(end_scores), [-1]),tf.float32))
